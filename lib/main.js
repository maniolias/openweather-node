(function() {
	...
	constructUrl = function(data,type,nb)
	{
		...
		{
			case 'string' :
				return	"/data/2.5/" + type + (forecastType && type == "forecast" ? "/" + forecastType: "") + "?q=" + data+ "&lang=" + lang + (units != "" ? ("&units=" + units):"") + (type == "forecast" ? "&cnt=" + (nb?nb:7):"") + (APPID?"&APPID=" + APPID : "") ;
			break;
			case 'number' :
				return	"/data/2.5/" + type + (forecastType && type == "forecast" ? "/" + forecastType : "") + "?id=" + data+ "&lang=" + lang + (units != "" ? ("&units=" + units):"") + (type == "forecast" ? "&cnt=" + (nb?nb:7):"") + (APPID?"&APPID=" + APPID : "") ;
			break;
			case 'object' :
				return	"/data/2.5/" + type + (forecastType && type == "forecast" ? "/" + forecastType : "") + "?lat=" + data[0] + "&lon=" + data [1]+ "&lang=" + lang + (units != "" ? ("&units=" + units):"") + (type == "forecast" ? "&cnt=" + (nb?nb:7):"") + (APPID?"&APPID=" + APPID : "") ;
			break;
		}
	} 
